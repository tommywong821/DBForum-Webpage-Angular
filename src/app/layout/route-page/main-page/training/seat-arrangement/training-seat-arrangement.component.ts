...
export class TrainingSeatArrangementComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.store
      .pipe(select(selectCurrentUserRole))
      .pipe(
        ...
        switchMap((paramMap) => {
          ...
          return forkJoin([
            this.dashboardRestful.getCoachList(),
            this.mainpageRestful.getTrainingDetail(this.trainingId),
          ]);
        }),
        ...
        switchMap(() =>
          this.mainpageRestful.getTrainingSearArr(this.trainingId)
        )
      )
      .subscribe({
        next: (seatArrList: any) => {
          seatArrList.forEach((seatArr: IDragonBoat) => {
            ...
            seatArr.right_seat = seatArr.right_seat.map((studentId: string) =>
              this.mapNFilterPlanStudent(studentId)
            );
            seatArr.left_seat = seatArr.left_seat.map((studentId: string) =>
              this.mapNFilterPlanStudent(studentId)
            );
            seatArr.steersperson = seatArr.steersperson.map(
              (studentId: string) => this.mapNFilterPlanStudent(studentId)
            );
            //push into drag and drop ui
            this.addDragonBoat(
              seatArr.left_seat,
              seatArr.right_seat,
              seatArr.steersperson,
              seatArr.dragon_boat_name
            );
          });
  ...
  addDragonBoat(
    ...
    this.dragonBoatFormArray = this.dragonBoatForm.get(
      'dragonBoatFormArray'
    ) as UntypedFormArray;
    this.dragonBoatFormArray.push(
      this.createDragonBoatWithId(dragonBoatId, dragonBoatName)
    );
  }

  calculateSideWeight(dragonId: any, position: any) {
    let dragonBoat: IDragonBoat = this.dragonBoats.filter(
      (dboat: IDragonBoat) => dboat.id == dragonId
    )[0];
    let studentList =
      position === 'left' ? dragonBoat.leftSeatList : dragonBoat.rightSeatList;
    return studentList
      .map((student) => student?.weight)
      .reduce((prev, curr) => this.sumFunction(prev, curr), 0);
  }
  ...
  calculateTotalWeight(dragonId: any) {
    ...
    return (
      dragonBoat.leftSeatList
        .map((student) => student?.weight)
        .reduce((prev, curr) => this.sumFunction(prev, curr), 0) +
      dragonBoat.rightSeatList
        .map((student) => student?.weight)
        .reduce((prev, curr) => this.sumFunction(prev, curr), 0)
    );
  }
  ...
  calculateTotalPeople(dragonId: any) {
    ...
    return (
      dragonBoat.leftSeatList.filter((student) => student).length +
  ...
  saveSeatArrangement() {
    ...
    this.dragonBoats.forEach((dragonBoat: IDragonBoat) => {
      dbObj.push({
        ...
        dragonBoatName: this.dragonBoatFormArray.value.filter(
          (dragonBoatFormObj: {
            dragonBoatId: any;
            id: any;
            dragonBoatName: any;
    ...
    this.mainpageRestful
      .updateTrainingSearArr(this.trainingId, dbObj)
      .subscribe({
        next: (value) => {
  ...
  mapNFilterPlanStudent(studentId: string) {
    let student = this.attendedRightStudentList.find(
      (student: any) => student.student_id == studentId
    );
    if (student) {
      //check in right list
      this.attendedRightStudentList = this.attendedRightStudentList.filter(
        (student: any) => student.student_id !== studentId
      );
    } else {
      //check in left list
      student = this.attendedLeftStudentList.find(
        (student: any) => student.student_id == studentId
      );
      this.attendedLeftStudentList = this.attendedLeftStudentList.filter(
        (student: any) => student.student_id !== studentId
      );
    }
    ...
    if (!student && this.coachList) {
      ...
      this.coachList = this.coachList.filter(
        (coach: any) => coach.uuid !== studentId
      );
    }
    return student;
  }
